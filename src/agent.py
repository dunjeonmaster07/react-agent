...
SYSTEM_PROMPT = "You are a helpful assistant. Use the provided tools when needed. Always use the correct tool calling format."


def agent_node(state: MessageState):
    # A function docstring here is never seen by the model — it has to be
    # passed as an actual message, so the system prompt is sent explicitly.
